...
describe Dessert do
  subject(:dessert) { Dessert.new('cheesecake', 4, chef) }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new('cookies', 'many', chef) }.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ["cream", "cheese", "egg", "sugar", "butter"]
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Guy")
      expect(dessert.serve).to eq("Guy has made 4 cheesecakes!")
    end
  end
